...
const tabColors = {
    ...
    theory: 'text-indigo-400',
    lab: 'text-green-400',
    quiz: 'text-yellow-400',
    resources: 'text-pink-400',
    about: 'text-teal-400'
};

// --- 2. THE FIX: MOBILE MENU LOGIC ---
...
function setActiveTab(page) {
    // Reset all tabs
    const colorClasses = Object.values(tabColors);
    document.querySelectorAll('.nav-tab').forEach(tab => {
        tab.classList.remove(...colorClasses, 'after:w-full');
    });

    // Set active desktop
    const desktopTab = document.getElementById(`tab-${page}`);
    if (desktopTab) desktopTab.classList.add(tabColors[page], 'after:w-full');

    // Set active mobile
    const mobileTab = document.getElementById(`mobile-tab-${page}`);
    if (mobileTab) mobileTab.classList.add(tabColors[page]);
...
function showSection(sectionId) {
    currentActiveView = sectionId;
    document.querySelectorAll('main > div, .view-section').forEach(sec => {
        sec.classList.add('hidden');
    });

    const targetId = (sectionId === 'quiz') ? 'view-quiz-home' : `view-${sectionId}`;
    ...
    if(sectionId === 'quiz' && typeof initQuizHome === 'function') initQuizHome();
    if(sectionId === 'theory' && typeof filterTheory === 'function') filterTheory();
}
...
{ num: 315, txt: "Engine: How to reduce GC pressure?", opts: ["Reuse objects","Avoid unnecessary allocations","Both","Cannot reduce"], ans: "Both", expl: "Reusing objects and minimizing allocations reduces garbage collection overhead." }
    ...
    function setActiveTab(page) {
        const tabColors = { home: 'text-purple-400', theory: 'text-indigo-400', lab: 'text-green-400', quiz: 'text-yellow-400', resources: 'text-pink-400', about: 'text-teal-400' };
        
        // UI reset
        document.querySelectorAll('.nav-tab').forEach(tab => {
            tab.classList.remove(...Object.values(tabColors), 'after:w-full', 'active');
        });

        const desktopTab = document.getElementById(`tab-${page}`);
        if (desktopTab) desktopTab.classList.add(tabColors[page], 'after:w-full', 'active');
    ...
    function showSection(sectionId) {
        currentActiveView = sectionId;
        document.querySelectorAll('main > div, .view-section').forEach(sec => sec.classList.add('hidden'));

        // Handle specific naming convention in HTML
        const targetId = (sectionId === 'quiz') ? 'view-quiz-home' : `view-${sectionId}`;
        const targetSection = document.getElementById(targetId);
        ...
        if (sectionId === 'quiz') initQuizHome();
    }

    function initTheoryGrid() {
        const grid = document.getElementById('theory-grid');
        if (!grid) return;
        const colorsArr = ["#6366f1", "#10b981", "#f59e0b", "#f43f5e", "#8b5cf6", "#22d3ee"];
        grid.innerHTML = '';
        theoryData.forEach((t, i) => {
            const card = document.createElement('div');
            card.className = 'theory-card glass p-6 rounded-2xl border-l-4 transition-all hover:scale-105';
            card.style.borderLeftColor = colorsArr[i % colorsArr.length];
            card.innerHTML = `<h4 class="font-bold text-lg mb-2 text-white">${t.title}</h4><p class="text-sm text-slate-400">${t.body}</p>`;
            grid.appendChild(card);
        });
    }
    ...
    function initQuizHome() {
        ...
        segments.forEach((s, i) => {
            ...
            btn.className = "flex items-center space-x-4 p-4 bg-[#1e293b]/50 border border-white/10 rounded-2xl text-left hover:bg-indigo-600/20 transition-all hover:scale-[1.02]";
            btn.innerHTML = `
                <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-indigo-500 rounded-lg text-white font-black">${i + 1}</div>
                <div class="flex-grow"><span class="text-white font-bold text-sm tracking-widest uppercase">${s.title}</span></div>`;
    ...
    function renderQuestion() {
        ...
        q.opts.forEach((opt, i) => {
            ...
            div.className = "option-ui glass p-5 rounded-2xl flex items-center space-x-4 cursor-pointer hover:bg-white/5 transition-all";
            div.id = `opt-${i}`;
    ...
    function processAnswer() {
        ...
        if (isCorrect) {
            ...
            document.getElementById(`opt-${selectedOpt}`).classList.add('correct-choice');
            showModal("✅", "Correct!", q.expl);
        } else {
            stats.wrong++; stats.streak = 0;
            document.getElementById(`opt-${selectedOpt}`).classList.add('wrong-choice');
            const correctIdx = q.opts.indexOf(q.ans);
            document.getElementById(`opt-${correctIdx}`).classList.add('correct-choice');
            showModal("❌", "Incorrect", q.expl);
        }
        updateStats();
    }

    function showModal(icon, title, body) {
        document.getElementById('expl-icon-popup').innerText = icon;
        document.getElementById('expl-title-popup').innerText = title;
        document.getElementById('expl-body-popup').innerText = body;
    ...
    function closeExplanation() {
        ...
        setTimeout(() => { modal.style.display = 'none'; handleNextQ(); }, 300);
    }

    function handleNextQ() {
        activeQuizIdx++;
        if (activeQuizIdx >= qBank.length) { showSection('quiz'); return; }
        quizState = "playing";
        renderQuestion();
    }

    function updateStats() {
        if(document.getElementById('stat-correct')) document.getElementById('stat-correct').innerText = stats.correct;
        if(document.getElementById('stat-wrong')) document.getElementById('stat-wrong').innerText = stats.wrong;
